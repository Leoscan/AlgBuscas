...
public class BuscaEstrelaV2 extends Algoritmo {
    ...
    public int[][] Busca(int[][] array, int[][] resultado) {
        ...
        while (!filaPrioridade.isEmpty()) {
            ...
            if (Arrays.deepEquals(estadoAtual.estado, resultado)) {
                ...
                Util.writeSolutionToHTML(solutionSteps, "Solucao_BuscaEstrelaV2.html");
    ...
    private static int calcularHeuristica(int[][] estado) {
        int heuristicaForaDoLugar = 0;
        int heuristicaDistanciaManhattan = 0;
        int objetivo = 1;

        for (int i = 0; i < estado.length; i++) {
            for (int j = 0; j < estado[i].length; j++) {
                if (estado[i][j] != objetivo) {
                    heuristicaForaDoLugar++;
                    // Calcula a distância de Manhattan para a peça atual
                    int valor = estado[i][j];
                    int objetivoLinha = (valor - 1) / 3;
                    int objetivoColuna = (valor - 1) % 3;
                    int distanciaManhattan = Math.abs(i - objetivoLinha) + Math.abs(j - objetivoColuna);
                    heuristicaDistanciaManhattan += distanciaManhattan;
                }
                objetivo++;
            }
        }

        // A heurística é a combinação das duas heurísticas
        return heuristicaForaDoLugar + heuristicaDistanciaManhattan;
    }
